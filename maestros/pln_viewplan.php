		...
		{ 
			...
			if($aCurso['niv_est'] != $vNiv_est or $aCurso['sem_anu'] != $vSem_anu)
			{
				...
				<tr <? if($vCont % 2 == 0) echo "class=\"celpar\""; else echo "class=\"celinpar\""; $vCont++;	?>>
				   <td class="wordcen">&nbsp;</td>
				   <td colspan="6" class="wordizqb">&nbsp;Nivel: <?=$_SESSION["sNivel{$aCurso['niv_est']}"]?> - Semestre: <?=$_SESSION["sSemestre{$aCurso['sem_anu']}"]?></td>
			    </tr>
				<?
			}
			...
			?>
				  <tr <? if($vCont % 2 == 0) echo "class=\"celpar\""; else echo "class=\"celinpar\""; $vCont++;	?>>
					<td class="wordcen">&nbsp;</td>
					<td colspan="6" class="wordizqb">&nbsp;Menci&oacute;n: <?=ucwords(strtolower($sEspecial[$aCurso['cod_esp']]['esp_nom']))?></td>
				  </tr>
			<?								
				}
			}
