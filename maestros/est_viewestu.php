...
<?php
	...
	if(fsafetylogin())
	{
		...
		if((!empty($vNum_mat) or !empty($vPaterno) or !empty($vMaterno) or !empty($vNombres)))
		{
			...
			$vQuery .= " order by paterno, materno, nombres limit 10 ";
			$bDatos = TRUE;
		}
	}
	else
...
<table border="1" cellpadding="1" cellspacing="0" bordercolor="#BDD37B" rules="cols, rows" class="tabled">
	...
	if($bDatos == TRUE)
	{
		...
		{
			$sUsercoo['num_mat'] = $aEstudia['num_mat'];
			$sUsercoo['cod_car'] = $aEstudia['cod_car'];
  	?>
  <tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
    ...
    <td width="60"><a href="" onClick="est_viewdataest('<?=$aEstudia['num_mat']?>', '<?=$aEstudia['cod_car']?>'); return false;" class="linktext">
      <?=$aEstudia['num_mat']?>
    </a></td>
    ...
    <td width="16" class="wordizq"><a href="" onClick="est_viewdataest('<?=$aEstudia['num_mat']?>', '<?=$aEstudia['cod_car']?>'); return false;" class="linkicon"><img src="../images/browse.png" alt="Mostrar informaci&oacute;n" width="16" height="16" /></a></td>
  </tr>
  	<? 
		$vCont++; 	
		} 
