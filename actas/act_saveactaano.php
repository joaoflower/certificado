...
<?
	...
	if(fsafetylogin())
	{
		...
		$tNota = "unapnet.nota{$_SESSION['sCod_car']}";
		$vResult = "";
		
		$vFch_rec = fFechamy($_POST['rFch_rec']);
		$aCod_act = $_POST['rCod_act'];
		
		$vCan_act = (strlen($aCod_act) / 6);
		
		for($ii = 0; $ii < $vCan_act;$ii++)
			$sActa[substr($aCod_act, $ii * 6, 6)] = substr($aCod_act, $ii * 6, 6);
		
		if(!empty($sActa)) foreach($sActa as $vCod_act2 => $vCod_act)
		{
			$vQuery = "update $tActa set ubi_act = '02' where cod_act = '$vCod_act'";
			$cResult = fInupde($vQuery);
			
			$vQuery = "Update $tNota set fch_reg = '$vFch_rec' where cod_act = '$vCod_act' and ";
			$vQuery .= "ano_aca = '{$_SESSION['sAno_aca']}'";
			$cResult = fInupde($vQuery);
		}
		//---------------------------------------------------------
...
?>
	...
	<table border="0" cellpadding="0" cellspacing="0" id="ventana">
	  <tr>
		...
		<th align="center" background="../images/ventana_r1_c2.jpg" >Listado de Actas Recepcionadas </th>
			...
			?>
			  ...
			  <?
				...
				<tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
				  ...
				  <td class="wordcen"><?=$aActa['ubi_act2']?></td>
				</tr>
			<? 		
				$vCont++; 	
			}
