...
<?
	...
	if(fsafetylogin())
	{
		...
		$vNom_cur = $_POST['rNom_cur'];
		$vCrd_cur = $_POST['rCrd_cur'];
		$vNiv_est = $_POST['rNiv_est'];
		$vSem_anu = $_POST['rSem_anu'];
		
		$vCod_cat = $_SESSION['sPln_est'].'-'.$vCod_cur;
		
		if($_SESSION['sAccion'] == "New")
		{
			$vQuery = "Insert into unapnet.curso (cod_cur, cod_car, pln_est, cod_cat, nom_cur, nom_ofi, sem_anu, ";
			$vQuery .= "cod_esp, crd_cur, dep_aca, cod_are, tip_pre, tip_cur, niv_est) values ";
			$vQuery .= "('$vCod_cur', '{$_SESSION['sCod_car']}', '{$_SESSION['sPln_est']}', '$vCod_cat', '$vNom_cur', ";
			$vQuery .= "'$vNom_cur', '$vSem_anu', '00', '$vCrd_cur', '', '', '01', '01', '$vNiv_est') ";
		}
		elseif($_SESSION['sAccion'] == "Edit")
		{
			$vQuery = "Update unapnet.curso set nom_cur = '$vNom_cur', nom_ofi = '$vNom_cur', sem_anu = '$vSem_anu', ";
			$vQuery .= "crd_cur = '$vCrd_cur', niv_est = '$vNiv_est' where cod_car = '{$_SESSION['sCod_car']}' and ";
			$vQuery .= "pln_est = '{$_SESSION['sPln_est']}' and cod_cur = '$vCod_cur'";
		}
		$cResult = fInupde($vQuery);
		if($cResult)
		{
			$_SESSION["sNiv_est"] = $vNiv_est;
			$_SESSION["sSem_anu"] = $vSem_anu;
		}
