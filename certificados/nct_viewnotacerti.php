	...
	if(fsafetylogin())
	{
		...
		{		
				...
				(!empty($_SESSION["sEstucod_esp"]) or !empty($_SESSION["sEstucod_esp2"])))
			{
				...
				$vQuery .= "where no.num_mat = '{$_SESSION['sEstunum_mat']}' and no.pln_est = '{$_SESSION['sEstupln_est']}' ";
				if($_SESSION["sEstutip_cer"] == '1')
					$vQuery .= "and no.not_cur > 10 ";
				$vQuery .= "order by cod_cur, not_cur ";
...
/*		$vQuery = "Select cu.cod_cur, cu.nom_ofi, cu.cod_esp, cu.crd_cur, cu.tip_cur, cu.niv_est, cu.sem_anu ";
		  ...
		  <?	
			...
			<tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
			  ...
			  <td class="wordcen"><?=fFechad($sNotas[$aCurso['cod_cur']]['fch_reg'])?></td>
  			  <td class="wordder"><span class ="<?=($sNotas[$aCurso['cod_cur']]['not_cur'] < 11?"notades":"notapro")?>"><?=$sNotas[$aCurso['cod_cur']]['not_cur']?></span>&nbsp;</td>
	...
	<a href="" title="Retroceder" class="linkboton" onClick = "nct_selectplan(<?=$_SESSION["sEstunum_mat"]?>, <?=$_SESSION["sEstucod_car"]?>); return false"><img src="../images/bback.png" width="100" height="24"></a><a href="prncerestufle<?=($_SESSION["sEstucod_car"]=='25'?'der':'')?>.php" title="Imprimir Certificado de Estudios" class="linkboton" target="frPdf"><img src="../images/bpcerestu.png" width="100" height="24"></a>	
<?
	}
?>
