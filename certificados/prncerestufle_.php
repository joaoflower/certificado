	...
	if(fsafetylogin())
	{
		class PDF extends FPDF
		{
			...
			function Titulo()
			{
				...
				$this->Cell(5);
				$this->Cell(40,4, "De la Facultad ", 0);
				$this->Cell(140,4, ": ".$_SESSION["sFacultad".$vCod_fac], 0);
				$this->Ln();
				
				$this->Cell(5);
				$this->Cell(40,4, "De la Escuela Profesional ", 0);
				...
				$this->Cell(5);
				if($_SESSION["sEstutip_sist"] == '1')
					$this->Cell(40,4, "De la Especialidad", 0);
				elseif($_SESSION["sEstutip_sist"] == '2')
					$this->Cell(40,4, "De la Mención", 0);
				
				if(strlen($vEsp_nom) > 30)
					$this->Cell(140,4, ": $vEsp_nom", 0);
				else
				{
					for($i = 30; $i > 0; $i--)
					{
						if(substr($vEsp_nom, $i, 1) == ' ')
							break;
					}
					$this->Cell(140,5,substr($vEsp_nom, 0, $i+1), $vLado);
					
					$this->Ln();
					$this->Cell(5);
					$this->Cell(40,4, "", 0);																								
					$vEsp_nom2 = "  ".substr($vEsp_nom, $i, strlen($vEsp_nom) - ($i));
					$this->Cell(140,4, ": $vEsp_nom2", 0);
				}
				$this->Ln();
				//-----------------
				...
				$this->Cell(5);
				$this->Cell(40,4, "Con N° de matrícula ", 0);
			...
			function SubFooter()
			{
				...
				$this->Ln();
				$this->Cell(180,4,"Cualquier enmendadura o anotación de la línea de cierre de la relación inválida la presente certificación.", 0);
				$this->Ln();
...
//							$this->Cell(93,4, substr($sPlan['nom_cur'], 0, $i+1), 0, 0, 'L');
				...
				if($vFilas < 83 or $vFilas > 83)
				{
					$vLimite = 82;
					if($vFilas == 40 or $vFilas == 82 or $vFilas == 122 or $vFilas == 164)
						$vLado = 'LRB';
					else
						$vLado = 'LR';
					$this->Cell(10,5,'', $vLado, 0, 'C');
					...
					//----------------------------------------
					if($vFilas == 82 or $ii == 82 or $vFilas == 164 or $ii == 164)
					{
						$vLado = 'LRB';
						$this->Cell(10,5,'', $vLado, 0, 'C');
						if($_SESSION["sEstutip_sist"] == '1')
					...
					//----------Linea de cierre (Cara 2)---------------
					if(($vFilas <= 41) or ($vFilas > 83 and $vFilas <= 123))
			...
			function Footer()
			{
				...
				$vFecha = getdate(time());
				$vFechan = "Puno, {$vFecha['mday']} de {$sMes[$vFecha['mon']]} del {$vFecha['year']}";
				$this->SetFont('arialn','',9);
