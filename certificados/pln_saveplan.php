...
/*		if($_SESSION["sCod_car"] != $_POST['rCod_car'])
		...
		$vTip_sist = $_POST['rTip_sist'];
		
		$vQuery = "Insert into unapnet.plan (pln_est, cod_car, tip_sist, des_pln ) values ";
		$vQuery .= "('{$_SESSION['sPln_estn']}', '{$_SESSION['sCod_car']}', '$vTip_sist', 'PLAN DE ESTUDIOS') ";
		$cResult = fInupde($vQuery);

		if($cResult)
		{
			$vQuery = "Insert into unapnet.especial (cod_esp, pln_est, cod_car, esp_nom ) values ";
			$vQuery .= "('00', '{$_SESSION['sPln_estn']}', '{$_SESSION['sCod_car']}', 'CARRERA PURA') ";
			$cResult = fInupde($vQuery);
		}
		$_SESSION["sPln_est"] = $_SESSION["sPln_estn"];
		$_SESSION["sPln_estn"] = "";
