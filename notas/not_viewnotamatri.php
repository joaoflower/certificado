...
/*				$sFechaA = "";
				...
				$vQuery .= "order by ano_aca, per_aca, pln_est, niv_est, sem_anu, cod_esp, cod_cur, ord_not ";
				$cNotas = fQuery($vQuery);
				while($aNotas = $cNotas->fetch_array())
				{ 
					if($vAno_aca != $aNotas['ano_aca'] or $vPer_aca != $aNotas['per_aca'])
					{
						$vAno_aca = $aNotas['ano_aca'];
						$vPer_aca = $aNotas['per_aca'];
						
			?>
			<tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
			  <td class="wordcen">&nbsp;</td>
			  <td colspan="9" class="wordizqb">A&Ntilde;O: <?=$vAno_aca?> - PERIODO: <?=$_SESSION["sPeriodo{$vPer_aca}per_des"]?></td>
		  </tr>
		  	<?
					}
			?>
			<tr <? if($vCont % 2 == 0) echo "class=\"celpar\" id=\"p\""; else echo "class=\"celinpar\" id=\"i\"";?> onMouseOver="mouseover(this)" onMouseOut="mouseout(this)">
			  ...
			  <td width="220" class="wordizq">&nbsp;<?=ucwords(strtolower($aNotas['nom_ofi']))?>&nbsp;</td>
